...
function DetailLayout({
  ...
  return <div className={cn("opportunity-card-page__grid", embedded && "opportunity-card-page__grid--embedded")}><div className="opportunity-card-page__main"><Card className={cn("opportunity-focus-card", animated && "opportunity-card-fade-up")}><div className="opportunity-focus-card__eyebrow"><Tag>{vm.typeLabel}</Tag>{!hidePublicActions ? <div className="opportunity-focus-card__toolbar"><IconButton type="button" label={isFavorite ? "?????? ??????????? ?? ??????????" : "????????? ???????????"} size="xl" className="opportunity-focus-card__toolbar-button ui-opportunity-mini-card__favorite" aria-pressed={isFavorite} active={isFavorite} data-opportunity-id={favoriteOpportunityId ?? undefined} onClick={toggleFavorite}><HeartIcon /></IconButton><div ref={menuRef} className={cn("opportunity-focus-card__menu", menuOpen && "is-open")}><IconButton type="button" label="??? ????????" size="xl" className="opportunity-focus-card__toolbar-button" aria-haspopup="menu" aria-expanded={menuOpen} onClick={onMenuToggle}><MoreIcon /></IconButton>{menuOpen ? <div className="opportunity-focus-card__menu-popover" role="menu" aria-label="???????? ? ?????????"><button type="button" className="opportunity-focus-card__menu-action" role="menuitem" onClick={onComplaint}>????????????</button><button type="button" className="opportunity-focus-card__menu-action" role="menuitem" onClick={onShare}>??????????</button></div> : null}</div></div> : null}</div><div className="opportunity-focus-card__copy"><h1 className="ui-type-h2">{item.title}</h1><p className="ui-type-body">{vm.metaLine}</p></div><dl className="opportunity-focus-card__facts"><div className="opportunity-focus-card__fact"><dt>???:</dt><dd>{vm.typeLabel}</dd></div><div className="opportunity-focus-card__fact"><dt>??????:</dt><dd>{vm.employmentLabel || "?? ??????"}</dd></div><div className="opportunity-focus-card__fact"><dt>?????:</dt><dd>{item.locationCity || "?? ??????"}</dd></div><div className="opportunity-focus-card__fact"><dt>?????:</dt><dd>{item.locationAddress || "?? ??????"}</dd></div><div className="opportunity-focus-card__fact"><dt>????????????:</dt><dd>{formatOpportunityDateTime(item.publishAt) || "?? ???????"}</dd></div><div className="opportunity-focus-card__fact"><dt>????:</dt><dd>{item.expireAt ? formatOpportunityDateTime(item.expireAt) : "?? ??????"}</dd></div></dl>{vm.summaryAccent ? <div className="opportunity-focus-card__details"><strong className="ui-type-h3">{vm.summaryAccent}</strong>{vm.summaryNote ? <p className="ui-type-body">{vm.summaryNote}</p> : null}</div> : null}{uniq(item.tags).length ? <div className="opportunity-focus-card__chips">{uniq(item.tags).map((tag) => <Tag key={tag} tone="accent">{tag}</Tag>)}</div> : null}{!hidePublicActions ? <div className="opportunity-focus-card__footer"><div className="opportunity-focus-card__watchers">{item.expireAt ? `?? ${formatOpportunityDateTime(item.expireAt)}` : "???? ?? ??????"}</div><div className="opportunity-focus-card__actions"><Button type="button" className="opportunity-focus-card__apply" onClick={onApply} disabled={applyState.status === "saving" || applyState.status === "success"}>{applyButtonLabel}</Button></div></div> : null}</Card>{!hidePublicActions && applyState.status === "error" ? <Alert tone="error" title="?? ??????? ????????? ??????" showIcon>{applyState.error}</Alert> : null}{!hidePublicActions && applyState.status === "success" ? <Alert tone="success" title={applyState.successTitle} showIcon>{applyState.successMessage}</Alert> : null}{ownerPanel}<Card className={cn("opportunity-media-panel", animated && "opportunity-card-fade-up opportunity-card-fade-up--delay-1")}><div className="opportunity-media-panel__header"><h2 className="ui-type-h4">{"Медиа"}</h2><p className="ui-type-caption">?????????, ??????? ???????? ????????? ? ???????? ???????????.</p></div>{m.length ? <div className="opportunity-story-card__intro">{m.map((entry, index) => entry.url ? <AppLink key={`${entry.title}-${index}`} href={entry.url} className="opportunity-card-page__more-link">{entry.title}</AppLink> : <p key={`${entry.title}-${index}`} className="ui-type-body">{entry.title}</p>)}</div> : <p className="ui-type-body">???????? ???? ?? ???????? ?????-????????? ? ???? ????????.</p>}</Card><Card className={cn("opportunity-story-card", animated && "opportunity-card-fade-up opportunity-card-fade-up--delay-2")}><div className="opportunity-story-card__intro">{intro.map((paragraph, index) => <p key={`${paragraph}-${index}`} className="ui-type-body">{paragraph}</p>)}</div><section className="opportunity-story-section"><div className="opportunity-story-section__header"><h2 className="ui-type-h3">{"О публикации"}</h2></div><ul className="opportunity-story-list"><li>{formatOpportunityDateTime(item.publishAt) || "???? ?? ???????"}</li><li>{locationLine(item) || "??????? ?? ???????."}</li><li>{item.expireAt ? `?? ${formatOpportunityDateTime(item.expireAt)}` : "???? ?? ??????"}</li></ul></section>{c.length ? <section className="opportunity-story-section"><div className="opportunity-story-section__header"><h2 className="ui-type-h3">{"Контакты"}</h2></div><ul className="opportunity-story-list">{c.map((entry) => <li key={`${entry.type}-${entry.value}`}>{entry.type === "email" ? <AppLink href={`mailto:${entry.value}`}>{entry.value}</AppLink> : entry.type === "phone" ? <AppLink href={`tel:${entry.value.replace(/\s+/g, "")}`}>{entry.value}</AppLink> : <AppLink href={entry.value}>{entry.value}</AppLink>}</li>)}</ul></section> : null}{uniq(item.tags).length ? <section className="opportunity-story-section"><div className="opportunity-story-section__header"><h2 className="ui-type-h2">{"Ключевые навыки"}</h2></div><div className="opportunity-skill-cloud">{uniq(item.tags).map((tag) => <Tag key={tag} tone="accent">{tag}</Tag>)}</div></section> : null}{hidePublicActions ? null : <><p className="ui-type-caption">{formatOpportunityDateTime(item.publishAt)}</p><div className="opportunity-story-card__bottom-actions"><Button type="button" className="opportunity-story-card__bottom-primary" onClick={onApply} disabled={applyState.status === "saving" || applyState.status === "success"}>{applyButtonLabel}</Button><Button type="button" variant="secondary" className="opportunity-story-card__bottom-secondary" onClick={onComplaint}>{item.opportunityType === "event" ? "???????????? ?? ???????" : "???????????? ?? ???????????"}</Button></div></>}</Card></div><aside className="opportunity-card-page__side"><Card className={cn("company-spotlight", animated && "opportunity-card-fade-up opportunity-card-fade-up--delay-1")}><div className="company-spotlight__company"><Avatar size="lg" initials={initials(item.companyName)} className="company-spotlight__avatar company-spotlight__avatar--brand" /><div className="company-spotlight__copy">{publicCompanyHref ? <AppLink href={publicCompanyHref} className="opportunity-card-page__more-link"><h2 className="ui-type-h4">{item.companyName}</h2></AppLink> : <h2 className="ui-type-h4">{item.companyName}</h2>}<p className="ui-type-body">{item.companyDescription || "???????? ???????? ???? ?? ?????????."}</p></div></div>{item.companyLegalAddress ? <p className="ui-type-body">{item.companyLegalAddress}</p> : null}{s.length ? <div className="opportunity-story-card__intro">{s.map((entry) => <AppLink key={entry.id} href={entry.url} className="opportunity-card-page__more-link">{entry.label}</AppLink>)}</div> : null}<div className="company-spotlight__footer">{publicCompanyHref ? <Button href={publicCompanyHref} variant="secondary" className="company-spotlight__recommend">{"Открыть профиль компании"}</Button> : null}<IconButton href={c[0]?.value ? `mailto:${c[0].value}` : "#contacts"} label="???????? ????????" variant="outline" size="xl" className="company-spotlight__message"><span aria-hidden="true">?</span></IconButton></div></Card><div className="opportunity-card-page__matches"><p className="ui-type-caption opportunity-card-page__matches-label">{"Вам могут подойти"}</p>{rel.length ? rel.map((relatedItem, index) => <OpportunityMiniCard key={relatedItem.id} variant="compact" item={{ id: relatedItem.id, ...getOpportunityMiniCardPresentation(relatedItem), company: relatedItem.companyName, note: relatedItem.locationAddress || "", chips: uniq(relatedItem.tags).slice(0, 3) }} className={cn("related-opportunity-entry", animated && `opportunity-card-fade-up opportunity-card-fade-up--delay-${index + 2}`)} detailAction={{ href: buildDetailHref(relatedItem), label: "Подробнее", variant: "secondary" }} />) : <Card><EmptyState title="???? ??? ??????? ??????????" description="?????? ??????????? ???????? ????? ?????????????, ????? ??????? ?????? ????." tone="neutral" compact /></Card>}<AppLink href={catalogHref} className="opportunity-card-page__more-link">{"Еще возможности в каталоге"}</AppLink></div></aside></div>;
}
...
export function OpportunityDetailCardApp() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!numericId(opportunityId)) {
        setState({ status: "error", item: null, related: [], error: new Error("???????? ??????????? ?? ???????."), source: "api" });
        return;
      }
      try {
  ...
  async function handleApply() {
    ...
    try {
      ...
      setApply(applyState("error", { error: error?.message ?? "?? ??????? ????????? ??????." }));
    }
  }
  ...
  async function openShareModal() {
    ...
    try {
      ...
      setShareContactsState(createShareContactsState("error", { error: error?.message ?? "?? ??????? ????????? ?????? ?????????." }));
    }
  }
  ...
  async function handleShareWithContact(contact) {
    ...
    const shareTitle = String(state.item?.title || "???????????").trim();
...
${currentUrl}`;
  ...
  async function saveOwner(saveMode) {
    ...
    try {
      ...
      setOwnerState({ isEditing: false, draft: createOpportunityDraft(refreshed), saveState: { status: "success", error: "", message: saveMode === "submit" ? "?????????? ?????????? ?? ?????????." : "???????? ????????." } });
    } catch (error) {
      setOwnerState((current) => ({ ...current, saveState: { status: "error", error: error?.message ?? "?? ??????? ????????? ??????????.", message: "" } }));
    }
  }
  ...
  const ownerPanel = item && isOwner && !previewMode ? (ownerState.isEditing ? <OwnerEditor draft={ownerState.draft} onChange={(field, value) => { if (field === "contacts") return changeDraftContacts(value); if (field === "media") return changeDraftMedia(value); changeDraft(field, value); }} onSaveDraft={() => saveOwner("draft")} onSubmit={() => saveOwner("submit")} onPreview={buildOpportunityPreviewRoute(item.id)} onClose={stopEditing} onArchive={archiveOwner} onDelete={deleteOwner} saving={ownerState.saveState.status === "saving"} /> : <Card className="opportunity-owner-panel"><div className="company-dashboard-stack"><div className="company-dashboard-list-item__top"><div><h2 className="ui-type-h3">?????????? ???????????</h2><p className="ui-type-caption">{translateModerationStatus(item.moderationStatus)}</p></div><Tag tone="accent">{translateModerationStatus(item.moderationStatus)}</Tag></div>{item.moderationReason ? <Alert tone="warning" title="??????? ????????" showIcon>{item.moderationReason}</Alert> : null}{capabilities?.canViewResponses ? <Button href="/company/dashboard/responses" variant="secondary">{"Открыть отклики"}</Button> : null}<div className="company-dashboard-panel__actions"><Button type="button" onClick={startEditing}>{"Редактировать"}</Button><Button type="button" variant="secondary" onClick={() => { if (typeof window !== "undefined") { window.location.href = buildOpportunityPreviewRoute(item.id); } }}>{"Просмотр публичной версии"}</Button>{capabilities?.canArchive ? <Button type="button" variant="secondary" onClick={archiveOwner}>????? ? ??????????/????????????</Button> : null}{capabilities?.canDelete ? <Button type="button" variant="ghost" onClick={deleteOwner}>???????</Button> : null}</div>{ownerState.saveState.status === "error" ? <Alert tone="error" title="???????? ?? ?????????" showIcon>{ownerState.saveState.error}</Alert> : null}{ownerState.saveState.status === "success" ? <Alert tone="success" title="????????? ?????????" showIcon>{ownerState.saveState.message}</Alert> : null}</div></Card>) : null;

  return <main className="opportunity-card-page" data-testid="opportunity-detail-page"><div className="opportunity-card-page__shell ui-page-shell"><PortalHeader navItems={NAV_ITEMS} currentKey="opportunities" actionHref="/candidate/profile" actionLabel="???????" className="opportunity-card-header opportunity-card-fade-up" />{state.status === "loading" ? <Loader label="????????? ???????? ???????????" surface /> : null}{state.status === "error" ? <Alert tone="error" title="?? ??????? ????????? ????????" showIcon>{state.error?.message ?? "?????????? ??????? ??????????? ?????."}</Alert> : null}{state.status === "ready" && state.item ? <DetailLayout item={state.item} related={state.related} applyState={apply} onApply={handleApply} hidePublicActions={isOwner && !previewMode} ownerPanel={ownerPanel} menuOpen={menuOpen} menuRef={menuRef} onMenuToggle={() => setMenuOpen((current) => !current)} onShare={async () => { setMenuOpen(false); await openShareModal(); }} onComplaint={() => { setMenuOpen(false); handleComplaint(); }} buildDetailHref={(entry) => buildOpportunityDetailRoute(entry.id)} catalogHref="/opportunities" /> : null}<Modal open={shareModalOpen} onClose={() => { setShareModalOpen(false); setShareBusyKey(""); }} title="?????????? ????????????" description="???????? ???????, ???????? ?????? ????????? ???????? ??????????." size="md" className="opportunity-share-modal">{shareContactsState.status === "loading" ? <Loader label="????????? ????????" /> : null}{shareContactsState.status === "error" ? <Alert tone="error" title="?? ??????? ????????? ????????" showIcon>{shareContactsState.error}</Alert> : null}{shareContactsState.status === "ready" && !shareContactsState.items.length ? <EmptyState eyebrow="???? ?????" title="?????? ????????? ???? ????" description="???????? ???????? ? ???????? ?????????, ? ??? ???????? ????? ??? ???????? ???????." tone="neutral" compact /> : null}{shareContactsState.status === "ready" && shareContactsState.items.length ? <div className="opportunity-share-modal__list">{shareContactsState.items.map((contact) => <div key={contact.id} className="opportunity-share-modal__item"><div className="opportunity-share-modal__identity"><Avatar initials={String(contact.name || "").slice(0, 2).toUpperCase() || "?"} shape="rounded" className="opportunity-share-modal__avatar" /><div className="opportunity-share-modal__copy"><strong>{contact.name}</strong><span>{contact.email || "??????? ??? email"}</span></div></div><Button type="button" variant="secondary" loading={shareBusyKey === String(contact.id)} disabled={Boolean(shareBusyKey) && shareBusyKey !== String(contact.id)} onClick={() => { handleShareWithContact(contact); }}>??????????</Button></div>)}</div> : null}</Modal></div></main>;
}

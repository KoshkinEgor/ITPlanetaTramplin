...
import { Alert, Avatar, Button, Card, Checkbox, EmptyState, FormField, IconButton, Input, Loader, OpportunityMiniCard, Select, Tag, Textarea } from "../shared/ui";
...
function contacts(value) { const parsed = json(value, []); return Array.isArray(parsed) ? parsed.map((item) => ({ type: String(item?.type || "link").toLowerCase(), value: String(item?.value || "").trim() })).filter((item) => item.value) : []; }
...
function DetailLayout({ item, related, applyState, onApply, hidePublicActions = false, ownerPanel = null, buildDetailHref = (entry) => buildOpportunityDetailRoute(entry.id), catalogHref = "/opportunities", animated = true, embedded = false }) {
...
export function OpportunityDetailCardApp() {
  ...
  const [companyProfileStatus, setCompanyProfileStatus] = useState("idle");
  const [ownerState, setOwnerState] = useState({ isEditing: false, draft: null, saveState: { status: "idle", error: "", message: "" } });

  useEffect(() => { document.body.classList.add(BODY_CLASS); return () => document.body.classList.remove(BODY_CLASS); }, []);
  ...
  useEffect(() => {
    if (authSession.status !== "authenticated" || authSession.user?.role !== "company") { setCompanyProfile(null); setCompanyProfileStatus("idle"); return; }
    const controller = new AbortController();
    async function loadProfile() {
      setCompanyProfileStatus("loading");
      try {
        const profile = await getCompanyProfile(controller.signal);
        if (!controller.signal.aborted) { setCompanyProfile(profile); setCompanyProfileStatus("ready"); }
      } catch (error) {
        if (!controller.signal.aborted) { setCompanyProfile(null); setCompanyProfileStatus(error instanceof ApiError && error.status === 401 ? "unauthorized" : "error"); }
      }
    }
    loadProfile();
    return () => controller.abort();
  ...
  useEffect(() => { if (item && isOwner && !previewMode && !ownerState.isEditing) setOwnerState((current) => ({ ...current, draft: createOpportunityDraft(item) })); }, [item, isOwner, previewMode, ownerState.isEditing]);

  async function handleApply() {
    setApply(applyState("saving"));
    if (state.source === "demo") { setApply(applyState("success", successCopy(state.item?.opportunityType))); return; }
    try {
      ...
      if (error instanceof ApiError && error.status === 409) { await refreshCandidateApplications({ force: true }).catch(() => {}); setApply(applyState("success", successCopy(state.item?.opportunityType, "existing"))); return; }
